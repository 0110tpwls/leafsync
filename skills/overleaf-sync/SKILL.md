...
| `review [path] [--stat]` | **Read-only `git diff`-style preview** of local vs Overleaf *right now*, grouped **outgoing** (your edits that would push), **incoming** (Overleaf changes you'd pull), and **conflict** (both sides). Unified diffs (`--- overleaf` / `+++ local`); `--stat` shows just filenames; `[path]` reviews one file. Fetches Overleaf content over the socket (no ZIP), writes nothing. Run it before `watch --push` to see exactly what you'd send. |
